    ...
    (() => {
        ...
        function initSliders() {
            ...
            function initializeSwiper() {
                if (window.innerWidth > 575 && document.querySelector(".reviews__slider")) if (!window.swiperInstance) window.swiperInstance = new core(".reviews__slider", {
                    modules: [ Navigation, EffectFade ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 2,
                    ...
                    on: {}
                }); else window.swiperInstance.update(); else if (window.swiperInstance) {
            ...
            if (document.querySelector(".slider-cart")) new core(".slider-cart", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".slider-summary__body")) new core(".slider-summary__body", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
        ...
        da.init();
        let searchOpen = document.querySelector(".search-header__button-mobile");
        let searchClose = document.querySelector(".search-header__close");
        searchOpen.addEventListener("click", (function() {
            let searchBody = document.querySelector(".mobile-search");
            searchBody.classList.add("_show");
            bodyLock();
        }));
        searchClose.addEventListener("click", (function() {
            let searchBody = document.querySelector(".mobile-search");
            searchBody.classList.remove("_show");
            bodyUnlock();
        }));
        ...
        let filterOpen = document.querySelector(".products__button");
        let filterClose = document.querySelector(".filter-products__close");
        let filter = document.querySelector(".products__filter");
        if (filterOpen && filterClose && filter) {
        ...
        var checkboxes = document.getElementsByClassName("checkbox-block");
        for (var i = 0; i < checkboxes.length; i++) {
            var script_checkbox = checkboxes[i].querySelector('input[type="checkbox"]');
            var script_link = checkboxes[i].querySelector(".checkbox-label a");
            script_link.addEventListener("click", createClickListener(script_checkbox));
        }
        function createClickListener(checkbox) {
            return function(event) {
                event.preventDefault();
                if (!checkbox.disabled) checkbox.checked = !checkbox.checked;
            };
        }
        var dropdown = document.querySelector(".search-header__dropdown");
        function checkWindowWidth() {
            if (window.innerWidth > 768) dropdown.setAttribute("data-simplebar", ""); else dropdown.removeAttribute("data-simplebar");
        }
        checkWindowWidth();
        window.addEventListener("resize", checkWindowWidth);
        let inputBodies = document.querySelectorAll(".form-input__body");
        inputBodies.forEach((bodyInput => {
            let input = bodyInput.querySelector(".input");
            input.addEventListener("focusin", (function() {
                input.parentElement.classList.add("_focus");
            }));
            input.addEventListener("focusout", (function() {
                if (input.value.trim() === "") input.parentElement.classList.remove("_focus");
            }));
        }));
        var mainHeader = document.querySelector(".header__main");
        var headerHeight = mainHeader.offsetHeight;
        var scrollThreshold = headerHeight * .9;
        var isFixed = false;
        ...
        function handleScroll() {
            ...
            if (scroll > scrollThreshold && !isFixed) {
                isFixed = true;
                mainHeader.classList.add("_header-scroll");
                mainHeader.style.marginBottom = headerHeight + "px";
                mainHeader.parentNode.insertBefore(headerPlaceholder, mainHeader);
            } else if (scroll <= scrollThreshold && isFixed) {
                isFixed = false;
                mainHeader.classList.remove("_header-scroll");
                mainHeader.style.marginBottom = "";
                mainHeader.parentNode.removeChild(headerPlaceholder);
            }
        }
        ...
        function openPopup() {
            ...
            bodyLock();
            if (popupElement) popupElement.classList.add("_popup-show");
        }
        function closePopup() {
            this.getAttribute("data-close");
            var popupElement = document.getElementById("popup");
            bodyUnlock();
            if (popupElement) popupElement.classList.remove("_popup-show");
        }
        var popupButtons = document.querySelector("[data-popup]");
        var closeButtons = document.querySelector("[data-close]");
        if (popupButtons && closeButtons) {
            popupButtons.addEventListener("click", openPopup);
            closeButtons.addEventListener("click", closePopup);
        }
        let decorationBlock = document.querySelectorAll(".decoration-basket");
        decorationBlock.forEach((blockDecoration => {
            let decorationHeader = blockDecoration.querySelector(".decoration-basket__header span");
            let decorationContent = blockDecoration.querySelector(".decoration-basket__content");
            decorationHeader.addEventListener("click", (function(event) {
                event.preventDefault();
                decorationContent.classList.toggle("_show");
            }));
            document.addEventListener("click", (function(event) {
                var target = event.target;
                var isClickInsideHeader = decorationHeader.contains(target);
                var isClickInsideContent = decorationContent.contains(target);
                if (!isClickInsideHeader && !isClickInsideContent) decorationContent.classList.remove("_show");
            }));
        }));
        let script_tabs = document.querySelectorAll("._tabs");
        for (let e = 0; e < script_tabs.length; e++) {
            let t = script_tabs[e];
            let r = t.querySelectorAll("._tabs-item");
            let o = t.querySelectorAll("._tabs-block");
            for (let e = 0; e < r.length; e++) {
                let t = r[e];
                t.addEventListener("click", (function(i) {
                    for (let e = 0; e < r.length; e++) r[e].classList.remove("_active"), o[e].classList.remove("_active");
                    t.classList.add("_active"), o[e].classList.add("_active"), i.preventDefault();
                }));
            }
        }
        window["FLS"] = false;
